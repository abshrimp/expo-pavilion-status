    ...
    try {
        ...
        if (res.ok) {
            ...
            if (typeof data.st === 'number') {
                _serverAnchorMs = data.st * 1000;
                _perfAnchorMs = (perfStart + perfEnd) / 2;
            }
        }
    } catch {
        // 失敗しても何もしない
...
export function serverNow() {
    if (_serverAnchorMs == null) return new Date();
    return new Date(_serverAnchorMs + (performance.now() - _perfAnchorMs));
}
